/*
 * This version uses instancing, but also normal vertices instead of billboards.
 * maybe this will speed things up compared to v2.
 */ 
...
function main(scene) {
  ...
  gl.clearColor(...viewParams.clearColor, 1.0); 
  ...
  /* grid */ 
  const gridData = draw_grid(viewParams.gridZ);
  const gridBuffers = {
  ...
  function render(now) {
    ...
    updateScene(gl, time, programInfo, buffers);
    time += deltaTime;
...
function updateScene(gl, time, programInfo, buffers)
{
  {
