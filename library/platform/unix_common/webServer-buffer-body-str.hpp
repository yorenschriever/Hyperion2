//this is a modified version of buffer_body that works with std::string instead of char*.
//this buffer is used when dynamically generating content (eg for the mapping json)
//the problem was that when i passed a char* that i had no place to free that char array anywhere.
//this was solved by making a buffer that works with std::string instead of char*.
//now the buffer is cleaned up automatically when the std::string goes out of scope. 
...
#ifndef BOOST_BEAST_HTTP_BUFFER_BODY_STR_HPP
#define BOOST_BEAST_HTTP_BUFFER_BODY_STR_HPP
    ...
    */
    ...
    public:
        ...
        put(ConstBufferSequence const& buffers,
            error_code& ec)
        {
            const char * body_data = body_.data.c_str();
            if(! body_data)
            {
            ...
            auto const bytes_transferred =
                net::buffer_copy(net::buffer(
                    body_data, body_.size), buffers);
            body_.data = body_.data.substr(bytes_transferred);
            body_.size = body_.data.size();
        ...
        get(error_code& ec)
        {
            ...
            if(body_.data.c_str())
            {
                ...
                return {{const_buffers_type{
                    body_.data.c_str(), body_.data.size()}, body_.more}};
            }
            if(body_.more)
            {
                BOOST_BEAST_ASSIGN_EC(ec, error::need_buffer);
...
operator<<(std::ostream& os, message<isRequest,
    buffer_body_str, Fields> const& msg) = delete;
#endif
